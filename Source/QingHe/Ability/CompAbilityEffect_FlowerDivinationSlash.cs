...
{
    public class CompProperties_AbilityFlowerDivinationSlash : CompProperties_AbilityEffect
    {
        ...
        public int divinationTicksCost = 180;
        public int impactDelayTicks = 30;

        public string disabledReason = "花神降临期间才能使用花神闪。";
        public string noLineOfSightToLandingMessage = "花神闪的落点需要在清荷视线内。";
        public string noLineOfSightToDirectionMessage = "花神闪的打击方向需要从落点可见。";
        public string invalidLandingMessage = "无法闪现到该位置。";
        public string chooseLandingLabel = "选择花神闪落点";
        public string chooseDirectionLabel = "选择花神闪方向";
        ...
        public CompProperties_AbilityFlowerDivinationSlash()
        {
            compClass = typeof(CompAbilityEffect_FlowerDivinationSlash);
        }
    }

    public class CompAbilityEffect_FlowerDivinationSlash : CompAbilityEffect, ICompAbilityEffectOnJumpCompleted
    {
        ...
        private const int FlowerDivinationSlashArcDurationTicks = 36;
        ...
        public new CompProperties_AbilityFlowerDivinationSlash Props => (CompProperties_AbilityFlowerDivinationSlash)props;

        public override bool GizmoDisabled(out string reason)
        {
            if (!IsDivinationActive(parent?.pawn))
            {
        ...
        public override bool Valid(LocalTargetInfo target, bool throwMessages = false)
        {
            ...
            if (!IsDivinationActive(caster))
            {
                if (throwMessages)
                {
                    Messages.Message(Props.disabledReason, caster, MessageTypeDefOf.RejectInput, historical: false);
                }
                return false;
            }
        ...
        private bool CanLand(Pawn caster, LocalTargetInfo target, out string reason)
        {
            ...
            {
                reason = Props.invalidLandingMessage;
                return false;
            }
            ...
            {
                reason = Props.noLineOfSightToLandingMessage;
                return false;
            }
        ...
        private bool CanChooseDirection(Map map, IntVec3 landing, LocalTargetInfo direction, out string reason)
        {
            ...
            {
                reason = Props.noLineOfSightToDirectionMessage;
                return false;
            }
        ...
        private static bool IsDivinationActive(Pawn pawn)
        {
            return FlowerCourtUtility.GetFlowerDivination(pawn)?.Active == true;
        }
        ...
        private void ResolveLandingImpact(Pawn caster, IntVec3 origin, IntVec3 landing, IntVec3 directionCell)
        {
            ...
            map.GetComponent<MapComponent_FlowerDivinationVisuals>()?.AddArc(landing, forward, Props.coneRadius, Props.coneAngleDegrees, FlowerDivinationSlashArcDurationTicks);
            map.GetComponent<MapComponent_FlowerDivinationVisuals>()?.AddDelayedImpact(caster, landing, directionCell, Props.impactDelayTicks, Props);
            FlowerCourtUtility.GetFlowerDivination(caster)?.ConsumeActiveTicks(Props.divinationTicksCost);
        }

        public static void ResolveDelayedConeImpact(Pawn caster, Map map, IntVec3 landing, IntVec3 directionCell, CompProperties_AbilityFlowerDivinationSlash props)
        {
            if (props == null)
            {
                return;
        ...
        private static void ResolveCone(Pawn caster, Map map, IntVec3 landing, IntVec3 directionCell, CompProperties_AbilityFlowerDivinationSlash props)
        {
            Vector3 forward = ComputeForward(landing, directionCell);
    ...
    public class Command_AbilityFlowerDivinationSlash : Command_Ability
    {
        public Command_AbilityFlowerDivinationSlash(Ability ability, Pawn pawn) : base(ability, pawn)
        {
        }
        ...
        public override void ProcessInput(Event ev)
        {
            ...
            CompAbilityEffect_FlowerDivinationSlash comp = Ability.CompOfType<CompAbilityEffect_FlowerDivinationSlash>();
            if (comp == null)
            {
                return;
            }
        ...
        private void BeginLandingTargeting(CompAbilityEffect_FlowerDivinationSlash comp)
        {
        ...
        private void BeginDirectionTargeting(CompAbilityEffect_FlowerDivinationSlash comp, LocalTargetInfo landing)
        {
            TargetingParameters parameters = TargetingParameters.ForCell();
            Find.Targeter.BeginTargeting(
                parameters,
        ...
        private static void DrawLandingMouseLabel(CompAbilityEffect_FlowerDivinationSlash comp, LocalTargetInfo target, Texture2D validIcon)
        {
            string reason;
            DrawMouseLabel(comp.Props.chooseLandingLabel, comp.CanLandForCommand(target, out reason) ? null : reason, validIcon);
        }

        private static void DrawDirectionMouseLabel(CompAbilityEffect_FlowerDivinationSlash comp, LocalTargetInfo landing, LocalTargetInfo direction, Texture2D validIcon)
        {
            string reason;
            DrawMouseLabel(comp.Props.chooseDirectionLabel, comp.CanChooseDirectionForCommand(landing, direction, out reason) ? null : reason, validIcon);
        }
